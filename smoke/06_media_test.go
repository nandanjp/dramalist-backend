package smoke_test

import (
	"bytes"
	"fmt"
	"net/http"
	"testing"
)

// TestMediaPresignFlow verifies the presigned URL upload path for profile images:
//   media-service presign → client PUT to MinIO → user-service avatar update.
func TestMediaPresignFlow(t *testing.T) {
	// ── Step 1: request a presigned URL ──────────────────────────────────────
	t.Log("step 1: requesting presigned upload URL")
	body, status := do(t, http.MethodPost, mediaURL()+"/media/presign",
		map[string]any{"content_type": "image/jpeg"},
		userHeaders(),
	)
	assertStatus(t, status, http.StatusOK, body)

	var presignResp struct {
		PresignedURL string `json:"presigned_url"`
		Key          string `json:"key"`
		ExpiresAt    string `json:"expires_at"`
	}
	decodeJSON(t, body, &presignResp)

	if presignResp.PresignedURL == "" {
		t.Fatal("presigned_url is empty")
	}
	if presignResp.Key == "" {
		t.Fatal("key is empty")
	}
	logf(t, "presigned URL received: key=%s", presignResp.Key)

	// ── Step 2: PUT a minimal JPEG directly to MinIO via the presigned URL ────
	t.Log("step 2: uploading image directly to MinIO via presigned URL")
	minimalJPEG := minimalJPEGBytes()
	req, err := http.NewRequest(http.MethodPut, presignResp.PresignedURL, bytes.NewReader(minimalJPEG))
	if err != nil {
		t.Fatalf("new PUT request: %v", err)
	}
	req.Header.Set("Content-Type", "image/jpeg")
	req.ContentLength = int64(len(minimalJPEG))

	resp, err := client.Do(req)
	if err != nil {
		t.Fatalf("PUT to MinIO: %v", err)
	}
	resp.Body.Close()
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusNoContent {
		t.Fatalf("MinIO PUT: expected 200/204, got %d", resp.StatusCode)
	}
	logf(t, "image uploaded to MinIO: key=%s", presignResp.Key)

	// ── Step 3: notify user-service of the new avatar ─────────────────────────
	t.Log("step 3: updating user avatar via user-service")
	body, status = do(t, http.MethodPatch, userURL()+"/users/me/avatar",
		map[string]any{"key": presignResp.Key},
		userHeaders(),
	)
	assertStatus(t, status, http.StatusOK, body)

	profileImageURL := jsonField(t, body, "profile_image_url")
	if profileImageURL == "" {
		t.Fatal("expected profile_image_url to be set after avatar update")
	}
	logf(t, "avatar updated: profile_image_url=%s", profileImageURL)
}

// TestMediaAdminUploadFlow verifies the existing multipart upload endpoint.
func TestMediaAdminUploadFlow(t *testing.T) {
	catalogID := requireCatalogEntry(t)

	t.Log("admin multipart upload for catalog poster")
	jpeg := minimalJPEGBytes()

	body := &bytes.Buffer{}
	// Build a minimal multipart form.
	boundary := "smokeboundary"
	body.WriteString(fmt.Sprintf("--%s\r\n", boundary))
	body.WriteString("Content-Disposition: form-data; name=\"entity_type\"\r\n\r\ncatalog\r\n")
	body.WriteString(fmt.Sprintf("--%s\r\n", boundary))
	body.WriteString(fmt.Sprintf("Content-Disposition: form-data; name=\"entity_id\"\r\n\r\n%s\r\n", catalogID))
	body.WriteString(fmt.Sprintf("--%s\r\n", boundary))
	body.WriteString("Content-Disposition: form-data; name=\"media_type\"\r\n\r\nposter\r\n")
	body.WriteString(fmt.Sprintf("--%s\r\n", boundary))
	body.WriteString("Content-Disposition: form-data; name=\"file\"; filename=\"poster.jpg\"\r\n")
	body.WriteString("Content-Type: image/jpeg\r\n\r\n")
	body.Write(jpeg)
	body.WriteString(fmt.Sprintf("\r\n--%s--\r\n", boundary))

	req, err := http.NewRequest(http.MethodPost, mediaURL()+"/media/upload", body)
	if err != nil {
		t.Fatalf("new request: %v", err)
	}
	req.Header.Set("Content-Type", fmt.Sprintf("multipart/form-data; boundary=%s", boundary))
	for k, v := range adminHeaders() {
		req.Header.Set(k, v)
	}

	resp, err := client.Do(req)
	if err != nil {
		t.Fatalf("do upload: %v", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusCreated {
		t.Fatalf("admin upload: expected 201, got %d", resp.StatusCode)
	}
	logf(t, "admin multipart upload succeeded")
}

// minimalJPEGBytes returns a valid 1×1 white JPEG for upload tests.
func minimalJPEGBytes() []byte {
	return []byte{
		,
	}
}
